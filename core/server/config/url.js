...
// Usage:
// createUrl('/', true) -> http://icollege.com/
// E.g. /blog/ subdir
// createUrl('/welcome-to-icollege/') -> /blog/welcome-to-icollege/
// Parameters:
...
// Usage:
// urlFor('home', true) -> http://icollege.com/
// E.g. /blog/ subdir
...
// urlFor('post', {...}) -> /welcome-to-icollege/
// E.g. if post object represents welcome post, and slugs are set to date
// urlFor('post', {...}) -> /2014/01/01/welcome-to-icollege/
// Parameters:
...
function urlFor(context, data, absolute) {
    var urlPath = '/',
        secure, imagePathRe,
        knownObjects = ['post', 'user', 'image'], baseUrl,
    ...
    if (_.isObject(context) && context.relativeUrl) {
        ...
        if (context === 'post' && data.post) {
            ...
            secure = data.secure;
        } else if (context === 'user' && data.user) {
            urlPath = '/user/' + data.author.slug + '/';
            secure = data.author.secure;
        } else if (context === 'image' && data.image) {
            urlPath = data.image;
            imagePathRe = new RegExp('^' + icollegeConfig.paths.subdir + '/' + icollegeConfig.paths.imagesRelPath);
            absolute = imagePathRe.test(data.image) ? absolute : false;
            secure = data.image.secure;

            if (absolute) {
                // Remove the sub-directory from the URL because icollegeConfig will add it back.
                urlPath = urlPath.replace(new RegExp('^' + icollegeConfig.paths.subdir), '');
                baseUrl = (secure && icollegeConfig.urlSSL) ? icollegeConfig.urlSSL : icollegeConfig.url;
